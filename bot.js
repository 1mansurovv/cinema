...
if (!token) {
  console.error("❌ BOT_TOKEN yo‘q! (.env yoki Railway Variables) ni tekshiring.");
  process.exit(1);
}
...
console.log("MOVIES_FILE:", MOVIES_FILE);
...
function ensureFile(filePath) {
  try {
    ...
    console.log("❌ ensureFile error:", filePath, e.message);
  }
}
...
function readJson(file) {
  try {
    ...
    console.log("⚠️ readJson error:", file, e.message);
    return {};
  }
}
function writeJson(file, data) {
  try {
    ...
    console.log("❌ writeJson error:", file, e.message);
  }
}
...
// webhook qolib ketgan bo‘lsa — polling uchun tozalaymiz
(async () => {
  try {
...
// ✅ Bot username (@siz)
const BOT_USERNAME = "cinemamod_bot";
